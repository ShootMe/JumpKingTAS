...
namespace TASStudio.Controls {
	public class RichText : UserControl {
		...
		private Color changedLineColor, currentLineColor, currentTextColor, activeLineColor, foldingIndicatorColor, indentBackColor, paddingBackColor, lineNumberColor, serviceLinesColor, selectionColor;
		...
		private bool mouseIsDrag, multiline, needRecalc, showLineNumbers, showFoldingLines, needRecalcFoldingLines, wordWrap, scrollBars, handledChar, highlightFoldingIndicator, isChanged;
		private Range rightBracketPosition, rightBracketPosition2, updatingRange, leftBracketPosition, leftBracketPosition2, delayedTextChangedRange, selection;
			...
			for (iLine = startLine; iLine < lines.Count; iLine++) {
				...
				if (lineInfo.VisibleState == VisibleState.Visible)
					if (line.BackgroundBrush != null)
						e.Graphics.FillRectangle(line.BackgroundBrush, new Rectangle(leftTextIndent, y, textWidth, CharHeight * lineInfo.WordWrapStringsCount));
				...
				if (!string.IsNullOrEmpty(currentLineText) && iLine == CurrentLine) {
					using (var lineNumberBrush = new SolidBrush(currentTextColor)) {
						...
						e.Graphics.DrawString(currentLineText, Font, lineNumberBrush, new RectangleF(ClientSize.Width - size.Width - 10, y, size.Width, CharHeight), StringFormat.GenericTypographic);
					}
				}
				...
				//OnPaint event
				if (lineInfo.VisibleState == VisibleState.Visible)
					OnPaintLine(new PaintLineEventArgs(iLine, new Rectangle(LeftIndent, y, Width, CharHeight * lineInfo.WordWrapStringsCount), e.Graphics, e.ClipRectangle));
				//draw line number
				if (ShowLineNumbers)
				...
				if (lineInfo.VisibleState == VisibleState.Visible && !string.IsNullOrEmpty(line.FoldingEndMarker) && string.IsNullOrEmpty(line.FoldingStartMarker))
					e.Graphics.DrawLine(servicePen, LeftIndentLine, y + CharHeight * lineInfo.WordWrapStringsCount - 1, LeftIndentLine + 4, y + CharHeight * lineInfo.WordWrapStringsCount - 1);
